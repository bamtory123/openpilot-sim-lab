...
A current-environment run is a separate compatibility check. It confirms that the documented configuration can still execute on the current host, but never replaces the historical baseline or moves its reference point. Before approval, an integrity audit must confirm every required per-run artifact and its provenance; a missing artifact remains an evidence gap rather than reconstructed evidence. Project-defined performance gates remain pending the [release process](../../docs/release-process.md).
