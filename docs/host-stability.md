...
The same VmSwitch warning occurred 257 times in the preceding 24 hours, including normal periods. The collector suppresses this ID 285 `OID_GEN_STATISTICS` message as background diagnostic noise; other matching VmSwitch Operational events remain collected.
...
On 2026-09-03 KST (2026-09-02T18:18Z), a fresh bounded host-stack artifact completed a 5-second CUDA soak (3,634 iterations), two offscreen MetaDrive steps, and preflight on RTX 4080 driver 616.56 without a boot-ID change. The following one 200-frame host-confirmation package was `valid/pass`: 9.99 seconds active time, 1.0 telemetry/road-camera coverage, 200 published frames, zero drops, and the same boot ID. The selected Windows event window (2026-09-02T18:18:46Z–18:20:28Z) retained zero events. These are bounded component/engagement/transport observations only; they do not clear the long CUDA-backed bridge boundary or qualify pretrained driving.
...
Summarize a retained collector JSON without assigning causality:

```bash
uv run python scripts/summarize_windows_wsl_events.py windows-host-events.json
```
