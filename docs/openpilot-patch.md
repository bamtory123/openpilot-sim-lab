...
| Control telemetry | model path horizon/end geometry (with 20 m values only when reached), model/planner/control curvature, openpilot steering command, normalized simulator steer, vehicle yaw-rate/curvature | separates perception/planning output from simulator actuation response |
