...
The follow-up diagnosis aligned 6,000 measurement frames across those repeats. Model-frame IDs matched throughout, specialist steer first exceeded the project-defined 0.005 spread at frame 262, and lateral/position spread first exceeded 5 cm at frame 3,571. A separate ±1 pixel/±2 luma test on 32 temporal pairs found an anchored-to-v0.6 P95 output-delta ratio of `0.836`; increased bounded static pixel sensitivity was not observed. The supported conclusion is insufficient closed-loop stability margin, not a proven renderer, scheduler, or model-sensitivity cause. No new augmentation candidate is justified by this evidence.
