...
- A further foreground 1,200-frame specialist host probe completed with 59.99 s active time, 1.0 telemetry/camera coverage, zero drop, valid timestamps, no departure/collision, and unchanged WSL boot ID. It is host-stability evidence for its fixed specialist contract only; it does not modify the pretrained v0.1 baseline or its qualification hard-gate.
