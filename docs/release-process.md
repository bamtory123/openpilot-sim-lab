...
The v0.1 comparison reference is the frozen historical 0 ms, three-run baseline. Audit its required per-run artifacts and provenance before approval. A missing required artifact is `evidence_gap`: do not reconstruct it or replace the reference with a current-host run. A replacement needs explicit approval of a new baseline version. Execute the separate two-run `md_default_loop_lane0_host_confirmation_v1` probe after the audit. It has the same SUT, map, seed and 0 ms transport path but only nominal 10 seconds/200 frames (9.5-second timestamp minimum) of engagement, transport and artifact-integrity scope; it is not a driving-performance result. Each run must be non-`invalid`; an infrastructure `invalid` leaves compatibility `not_confirmed`. Record manifests and outcomes without letting them replace or redefine the historical baseline.

Baseline audit and regression-review automation are implemented. The contract is [acceptance.yaml](../baselines/md_default_loop_lane0_v1/acceptance.yaml); Phase 3 performance gates remain null until project-defined thresholds are approved.
