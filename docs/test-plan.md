...
| TC-COMP-001 | Audit frozen baseline completeness, then run one current-host confirmation without changing the baseline. | ENV-002 | planned |
...
| TC-REG-001 | Compare baseline KPI deltas; emit review-required until performance gates are approved. | REG-001, REG-002 | planned |
