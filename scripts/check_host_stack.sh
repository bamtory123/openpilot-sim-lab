...
write_host_stack_result() {
  ...
  "$OPENPILOT_PYTHON" - "$host_stack_output" "$status" "$exit_code" "$stage" "$boot_before" "$boot_after" "$cuda_json" "$renderer_json" <<'PY'
import json
from pathlib import Path
import sys

path, status, exit_code, stage, boot_before, boot_after, cuda, renderer = sys.argv[1:]
