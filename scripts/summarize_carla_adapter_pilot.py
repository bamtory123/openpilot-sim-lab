...
def main() -> None:
  ...
  for path in sorted(args.root.glob("carla-city-mixed-pilot-*/summary.json")):
    if args.after and args.after not in path.parent.name and path.parent.name.split("-")[4] <= args.after:
      continue
  ...
  result = {"schema_version": 1, "scope": "carla_v02_adapter_pilot_not_road_qualification", "run_count": len(summaries),
            ...
            "reason_counts": dict(Counter(reason for row in summaries for reason in row["reasons"])), "runs": summaries}
